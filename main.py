...
for i in range(try_times):
    ...
    try:
        ...
        # 登录
        print('正在登录...')
        login_url = f'{root_url}/signin?c={random.random()}'
        res = session.post(login_url, data={'email': email, 'passwd': passwd})
        print('\n\t', res.json(), '\n')
        # 签到
        print('正在签到...')
        chekin_url = f'{root_url}/user/checkin?c={random.random()}'
